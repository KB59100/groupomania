    ...
    <div class="card mb-4 card-profile">
      <div class="card-body text-center">
        ...
        <div class="d-flex justify-content-center mb-2 card-btns">
          <router-link
            :to="`/modify-profile/${user.id}`"
            class="btn btn-primary btn-modify"
            v-if="loggedIn"
            >Editer le profile</router-link
          >
          <router-link
            :to="`/modify-password/${user.id}`"
            class="btn btn-primary btn-password"
            v-if="loggedIn"
            >Changer le mot de passe</router-link
          >
          <button @click="deleteUser" class="btn btn-groupo">
            Supprimer le compte
          </button>
        </div>
      </div>
  ...
  methods: {
    ...
    async getUser() {
      ...
      try {
        ...
        this.$refs.bio.innerHTML =
          this.bio !== null ? getClickableLink(response.data.bio) : "";
