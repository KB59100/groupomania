<template>
  <div class="card d-flex bg-white p-3 border-0 like-item">
    <div
      class="like-btn-container py-1 d-flex align-items-center justify-content-between"
    >
      <button
        type="button"
        @click="handleLike"
        class="p-2 like-btn border-0 d-flex justify-content-center align-items-center"
      >
        <i v-if="!isLiked" class="far fa-heart like-icon me-2" ref="icon"></i>
        <i v-if="isLiked" class="fas fa-heart like-icon me-2" ref="icon"></i>
        <span v-if="!isLiked" class="like-text color-grey" ref="text"
          >I like</span
        >
        <span v-if="isLiked" class="like-text color-groupo" ref="text"
          >Like</span
        >
      </button>
      <span class="count-like p-2">{{ countLikes }} Like</span>
    </div>
  </div>
</template>
